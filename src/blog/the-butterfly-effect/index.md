...
We always knew we could improve performance in the linter rule. Like in other parts of Nx, we keep finding ways to shave off another 100ms here and there. Improving performance has been an ongoing process. 
...
The last line  - `Array.from(new Set(…))` uses the abovementioned trick to filter out duplicates.
...
Both were iterating our graph, looking for a project that contained a given file (import target was either mapped to file if it was a relative import or mapped to barrel entry file in case of cross-project imports).
...
## Credits
Thanks to Stefan Van de Vooren, who helped benchmark the improvements on his company's repository and provided valuable inputs whose company brought to our attention the severity of the performance issue on the linter.
